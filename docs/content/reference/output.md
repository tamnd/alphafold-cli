...
describes what users get. Pick a format with `-o`, or let alphafold choose:
a table when writing to a terminal, JSONL when piped.

## Formats

```bash
alphafold <command> -o table   # aligned columns for reading
alphafold <command> -o jsonl   # one JSON object per line, for piping
alphafold <command> -o json    # a single JSON array
alphafold <command> -o csv     # spreadsheet friendly
alphafold <command> -o tsv     # tab-separated
alphafold <command> -o url     # just the URL column
alphafold <command> -o raw     # the underlying bytes, unformatted
```
...
```bash
alphafold <command> --fields id,title,url
```
...
```bash
alphafold <command> --template '{{.URL}} {{.Title}}'
```
...
```bash
alphafold <command>            # a table, because this is a terminal
alphafold <command> | wc -l    # JSONL, because this is a pipe
```
